...
template <RopeKernelMode Mode, int QHeads, int KHeads, class Coefficients = NativeRopeCoefficients>
void launch_fixed(const Tensor& positions, Tensor* q, Tensor* k, cudaStream_t stream, const Coefficients& coefficients = {}) {
    const int tokens = positions.ne[0];
    ...
    launch_fixed_block<Mode, QHeads, KHeads>(positions, q, k, block, stream, coefficients);
}

template <int HeadsPerBlock, int QHeads, int KHeads, class Coefficients = NativeRopeCoefficients>
void launch_dflash_split(const Tensor& positions, Tensor* q, Tensor* k, cudaStream_t stream, const Coefficients& coefficients = {}) {
    ...
    const int tokens      = positions.ne[0];
    rope_fixed_split_kernel<RopeKernelMode::DflashText1D, QHeads, KHeads, HeadsPerBlock, Coefficients>
...
bool launch_fixed_pair(const Tensor& positions, int rotary_dim, float theta, Tensor& q, Tensor& k,
    ...
    if (axes == 1 && q.ne[0] == 128 && rotary_dim == 128 && (std::is_same_v<Coefficients, PreparedRopeCoefficients> || theta == 1.0e7F) && q.ne[1] == 32 &&
        k.ne[1] == 8) {
        const int tokens = positions.ne[0];
        if (tokens <= 16) {
            ...
            launch_fixed_block<RopeKernelMode::DflashText1D, 32, 8>(positions, &q, &k, 160, stream, coefficients);
        }
        return true;
    }
    if (rotary_dim == 64 && (std::is_same_v<Coefficients, PreparedRopeCoefficients> || theta == 1.0e7F)) {
        if (q.ne[1] == 24 && k.ne[1] == 4) {
            if (axes == 1) {
                launch_fixed<RopeKernelMode::Text1D, 24, 4>(positions, &q, &k, stream, coefficients);
                return true;
            }
            if (axes == 3) {
                launch_fixed<RopeKernelMode::TextMrope, 24, 4>(positions, &q, &k, stream, coefficients);
                return true;
            }
        }
        if (q.ne[1] == 16 && k.ne[1] == 2) {
            if (axes == 1) {
                launch_fixed<RopeKernelMode::Text1D, 16, 2>(positions, &q, &k, stream, coefficients);
                return true;
            }
            if (axes == 3) {
                launch_fixed<RopeKernelMode::TextMrope, 16, 2>(positions, &q, &k, stream, coefficients);
                return true;
            }
        }
    }
    if (axes == 2 && rotary_dim == 72 && theta == 10'000.0F && q.ne[1] == 16 && k.ne[1] == 16) {
        launch_fixed<RopeKernelMode::Vision2D, 16, 16>(positions, &q, &k, stream, coefficients);
        return true;
    }
    return false;
}
...
void launch_fixed_single(const Tensor& positions, Tensor& x, cudaStream_t stream, const Coefficients& coefficients = {}) {
    launch_fixed<Mode, Heads, 0>(positions, &x, nullptr, stream, coefficients);
}

template <int Heads, class Coefficients = NativeRopeCoefficients>
bool launch_text_single(const Tensor& positions, int axes, Tensor& x, cudaStream_t stream, const Coefficients& coefficients = {}) {
    if (x.ne[1] != Heads) { return false; }
    if (axes == 1) {
        launch_fixed_single<RopeKernelMode::Text1D, Heads>(positions, x, stream, coefficients);
        return true;
    }
    if (axes == 3) {
        launch_fixed_single<RopeKernelMode::TextMrope, Heads>(positions, x, stream, coefficients);
        return true;
    }
    return false;
}
...
bool launch_fixed_single_dispatch(const Tensor& positions, int rotary_dim, float theta, Tensor& x,
    ...
    const int axes = positions.ne[1];
    if (axes == 1 && x.ne[0] == 128 && rotary_dim == 128 && (std::is_same_v<Coefficients, PreparedRopeCoefficients> || theta == 1.0e7F)) {
        if (x.ne[1] == 32) {
            launch_fixed_single<RopeKernelMode::DflashText1D, 32>(positions, x, stream, coefficients);
            return true;
        }
        if (x.ne[1] == 8) {
            launch_fixed_single<RopeKernelMode::DflashText1D, 8>(positions, x, stream, coefficients);
            return true;
        }
    }
    if (rotary_dim == 64 && (std::is_same_v<Coefficients, PreparedRopeCoefficients> || theta == 1.0e7F)) {
        if (launch_text_single<24>(positions, axes, x, stream, coefficients) ||
            launch_text_single<4>(positions, axes, x, stream, coefficients) ||
            launch_text_single<16>(positions, axes, x, stream, coefficients) ||
            launch_text_single<2>(positions, axes, x, stream, coefficients)) {
            return true;
        }
    }
    if (axes == 2 && rotary_dim == 72 && theta == 10'000.0F && x.ne[1] == 16) {
        launch_fixed_single<RopeKernelMode::Vision2D, 16>(positions, x, stream, coefficients);
        return true;
    }
    return false;
}
...
void rope_prepared_launch(const Tensor& positions, const PreparedRope& prepared,
                          Tensor& q, Tensor* k, cudaStream_t stream) {
    const PreparedRopeCoefficients coefficients{prepared};
    const bool fixed = k != nullptr
        ? launch_fixed_pair(positions, prepared.rotary_dim, prepared.theta, q, *k, stream, coefficients)
        : launch_fixed_single_dispatch(positions, prepared.rotary_dim, prepared.theta, q, stream, coefficients);
    if (!fixed) {
        launch_generic(positions, prepared.rotary_dim, prepared.theta, &q, k, stream, coefficients);
    }
    CUDA_CHECK(cudaGetLastError());
}
