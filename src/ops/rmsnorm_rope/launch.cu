...
void rmsnorm_rope_prepared_launch(const Tensor& positions, const Tensor* q_norm_weight,
    ...
    if (q != nullptr) {
        launch_fixed<true>(positions, q_norm_weight, k_norm_weight, q, k, tokens, stream, coefficients);
    } else {
        launch_fixed<false>(positions, nullptr, k_norm_weight, nullptr, k, tokens, stream, coefficients);
    }
    CUDA_CHECK(cudaGetLastError());
}
